...
export default function OverdueTradesReportsScreen() {
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      const response = await reportAPI.getOverdueTrades();
  ...
  const handleExport = async (format: 'pdf' | 'excel') => {
    ...
    try {
      ...
      const fileName = format === 'excel' ? 'overdue-trades-report.xls' : `overdue-trades-report.${format}`;
      ...
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(fileUri, {
          ...
          dialogTitle: `Overdue Trades Report - ${format.toUpperCase()}`,
        });
      }
  ...
  const getOverdueColor = (daysOverdue: number) => {
    if (daysOverdue <= 7) return '#F39C12';
    if (daysOverdue <= 15) return '#E67E22';
    return '#E74C3C';
  };
    ...
    <Card style={styles.reportCard}>
      <View style={styles.reportHeader}>
        <View style={styles.reportIcon}>
          <Ionicons
            name="warning"
            size={24}
            color={getOverdueColor(item.days_overdue)}
          />
        </View>
      ...
      <View style={styles.overdueInfo}>
        <View style={styles.overdueRow}>
          <Text style={styles.overdueLabel}>Loading Due Date:</Text>
          <Text style={styles.overdueDate}>{formatDate(item.loading_due_date)}</Text>
        </View>
        <View style={styles.overdueRow}>
          <Text style={styles.overdueLabel}>Days Overdue:</Text>
          <Text style={[styles.overdueDays, { color: getOverdueColor(item.days_overdue) }]}>
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Overdue Trades</Text>
          ...
          {reports.length > 0 ? (
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={true}>
                <View style={styles.tableContainer}>
                  ...
                  <View style={styles.tableHeader}>
                    ...
                    <Text style={[styles.headerCell, styles.overdueCell]}>Days Overdue</Text>
                  </View>
                  ...
                  {reports.map((item, index) => (
                    <View key={item.id} style={styles.tableRow}>
                      ...
                      <Text style={[
                        styles.cell,
                        styles.overdueCell,
                        { color: getOverdueColor(item.days_overdue), fontWeight: '600' }
                      ]}>
                        {item.days_overdue} days
                      </Text>
                    </View>
            ...
            <Card>
              <View style={styles.emptyContent}>
                ...
                <Text style={styles.emptyText}>No overdue trades found</Text>
              </View>
            </Card>
          )}
        </View>
