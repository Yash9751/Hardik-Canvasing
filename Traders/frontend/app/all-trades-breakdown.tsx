...
export default function AllTradesBreakdownScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      let response = await plusMinusAPI.getAll({ date: targetDate });
      ...
      if (!response.data || response.data.length === 0) {
        ...
        response = await plusMinusAPI.getAll({ date: targetDate });
      }
      
      // Transform the array to the expected structure
      const rows = response.data;
      if (Array.isArray(rows)) {
        const productsData = rows.map(row => ({
          product_type: row.nick_name || row.item_name,
      ...
      console.error('All trades breakdown error:', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  if (loading) {
    return (
      <SafeAreaView style={styles.loadingContainer}>
        <Text style={styles.loadingText}>Loading All Trades breakdown...</Text>
      </SafeAreaView>
    );
  }
      ...
      >
        ...
        <Card style={styles.dateCard}>
          <View style={styles.dateHeader}>
            ...
            <Text style={styles.dateText}>{formatDate(date || new Date().toISOString().split('T')[0])}</Text>
          </View>
        </Card>
          ...
          {products && products.length > 0 ? (
            ...
            <Card style={styles.emptyCard}>
              ...
              <Text style={styles.emptySubtext}>
                Generate a report to see profit and loss details
              </Text>
            </Card>
          )}
        </View>
